// 🎯 同步黃經理 Python 原廠內建的大金規格資料庫
// 🎯 大金全系列實體型號與冷房能力資料庫 (含系列別、型式對應與定價)
export const EQUIPMENT_DB = {
  "RA": [
    {
      "model": "FTHF20ZVLT",
      "cap": 2.2,
      "series": "SUPER MULTI系列",
      "unit_type": "壁掛式",
      "price": 12980.0
    },
    {
      "model": "FTHF25ZVLT",
      "cap": 2.5,
      "series": "SUPER MULTI系列",
      "unit_type": "壁掛式",
      "price": 14080.0
    },
    {
      "model": "FTHF30ZVLT",
      "cap": 3.0,
      "series": "SUPER MULTI系列",
      "unit_type": "壁掛式",
      "price": 15380.0
    },
    {
      "model": "FTHF40ZVLT",
      "cap": 4.1,
      "series": "SUPER MULTI系列",
      "unit_type": "壁掛式",
      "price": 18280.0
    },
    {
      "model": "FTHF50ZVLT",
      "cap": 5.0,
      "series": "SUPER MULTI系列",
      "unit_type": "壁掛式",
      "price": 21520.0
    },
    {
      "model": "FTHF60ZVLT",
      "cap": 6.0,
      "series": "SUPER MULTI系列",
      "unit_type": "壁掛式",
      "price": 24320.0
    },
    {
      "model": "FTHF71ZVLT",
      "cap": 7.2,
      "series": "SUPER MULTI系列",
      "unit_type": "壁掛式",
      "price": 27320.0
    },
    {
      "model": "FTXV22UVLT",
      "cap": 2.2,
      "series": "大關U系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV28UVLT",
      "cap": 2.8,
      "series": "大關U系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV36UVLT",
      "cap": 3.5,
      "series": "大關U系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV41UVLT",
      "cap": 4.1,
      "series": "大關U系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV50UVLT",
      "cap": 5.0,
      "series": "大關U系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV60UVLT",
      "cap": 6.0,
      "series": "大關U系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV71UVLT",
      "cap": 7.2,
      "series": "大關U系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV22ZVLT",
      "cap": 2.2,
      "series": "大關Z系列",
      "unit_type": "壁掛式",
      "price": 14080.0
    },
    {
      "model": "FTXV28ZVLT",
      "cap": 2.8,
      "series": "大關Z系列",
      "unit_type": "壁掛式",
      "price": 16180.0
    },
    {
      "model": "FTXV36ZVLT",
      "cap": 3.5,
      "series": "大關Z系列",
      "unit_type": "壁掛式",
      "price": 17480.0
    },
    {
      "model": "FTXV41ZVLT",
      "cap": 4.1,
      "series": "大關Z系列",
      "unit_type": "壁掛式",
      "price": 19980.0
    },
    {
      "model": "FTXV50ZVLT",
      "cap": 5.0,
      "series": "大關Z系列",
      "unit_type": "壁掛式",
      "price": 23820.0
    },
    {
      "model": "FTXV60ZVLT",
      "cap": 6.0,
      "series": "大關Z系列",
      "unit_type": "壁掛式",
      "price": 27220.0
    },
    {
      "model": "FTXV71ZVLT",
      "cap": 7.2,
      "series": "大關Z系列",
      "unit_type": "壁掛式",
      "price": 30220.0
    },
    {
      "model": "FTXM22YVLT",
      "cap": 2.2,
      "series": "家用MULTI系列",
      "unit_type": "壁掛式",
      "price": 17280.0
    },
    {
      "model": "FDXV22RVLT",
      "cap": 2.2,
      "series": "家用MULTI系列",
      "unit_type": "吊隱式",
      "price": 16680.0
    },
    {
      "model": "FTXM28YVLT",
      "cap": 2.8,
      "series": "家用MULTI系列",
      "unit_type": "壁掛式",
      "price": 19080.0
    },
    {
      "model": "FDXV28RVLT",
      "cap": 2.8,
      "series": "家用MULTI系列",
      "unit_type": "吊隱式",
      "price": 17980.0
    },
    {
      "model": "FTXM36YVLT",
      "cap": 3.6,
      "series": "家用MULTI系列",
      "unit_type": "壁掛式",
      "price": 22080.0
    },
    {
      "model": "FDXV36RVLT",
      "cap": 3.6,
      "series": "家用MULTI系列",
      "unit_type": "吊隱式",
      "price": 20280.0
    },
    {
      "model": "FTXM41YVLT",
      "cap": 4.1,
      "series": "家用MULTI系列",
      "unit_type": "壁掛式",
      "price": 24380.0
    },
    {
      "model": "FDXV41RVLT",
      "cap": 4.1,
      "series": "家用MULTI系列",
      "unit_type": "吊隱式",
      "price": 22080.0
    },
    {
      "model": "FTXM50YVLT",
      "cap": 5.0,
      "series": "家用MULTI系列",
      "unit_type": "壁掛式",
      "price": 27720.0
    },
    {
      "model": "FDXV50RVLT",
      "cap": 5.0,
      "series": "家用MULTI系列",
      "unit_type": "吊隱式",
      "price": 25020.0
    },
    {
      "model": "FTXM60YVLT",
      "cap": 6.0,
      "series": "家用MULTI系列",
      "unit_type": "壁掛式",
      "price": 30220.0
    },
    {
      "model": "FDXV60RVLT",
      "cap": 6.0,
      "series": "家用MULTI系列",
      "unit_type": "吊隱式",
      "price": 25620.0
    },
    {
      "model": "FTXM71YVLT",
      "cap": 7.1,
      "series": "家用MULTI系列",
      "unit_type": "壁掛式",
      "price": 33520.0
    },
    {
      "model": "FDXV71RVLT",
      "cap": 7.1,
      "series": "家用MULTI系列",
      "unit_type": "吊隱式",
      "price": 27720.0
    },
    {
      "model": "FTXM80RVLT",
      "cap": 8.0,
      "series": "家用MULTI系列",
      "unit_type": "壁掛式",
      "price": 34100.0
    },
    {
      "model": "FTXM90RVLT",
      "cap": 9.0,
      "series": "家用MULTI系列",
      "unit_type": "壁掛式",
      "price": 38100.0
    },
    {
      "model": "FTXV22XVLT",
      "cap": 2.2,
      "series": "橫綱X系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV28XVLT",
      "cap": 2.8,
      "series": "橫綱X系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV36XVLT",
      "cap": 3.5,
      "series": "橫綱X系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV41XVLT",
      "cap": 4.1,
      "series": "橫綱X系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV50XVLT",
      "cap": 5.0,
      "series": "橫綱X系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV60XVLT",
      "cap": 6.0,
      "series": "橫綱X系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV71XVLT",
      "cap": 7.2,
      "series": "橫綱X系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV80XVLT",
      "cap": 8.0,
      "series": "橫綱X系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXV90XVLT",
      "cap": 8.7,
      "series": "橫綱X系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXM22ZVLT",
      "cap": 2.2,
      "series": "橫綱Z系列",
      "unit_type": "壁掛式",
      "price": 18180.0
    },
    {
      "model": "FTXM28ZVLT",
      "cap": 2.8,
      "series": "橫綱Z系列",
      "unit_type": "壁掛式",
      "price": 19980.0
    },
    {
      "model": "FTXM36ZVLT",
      "cap": 3.5,
      "series": "橫綱Z系列",
      "unit_type": "壁掛式",
      "price": 23680.0
    },
    {
      "model": "FTXM41ZVLT",
      "cap": 4.1,
      "series": "橫綱Z系列",
      "unit_type": "壁掛式",
      "price": 24780.0
    },
    {
      "model": "FTXM50ZVLT",
      "cap": 5.0,
      "series": "橫綱Z系列",
      "unit_type": "壁掛式",
      "price": 27920.0
    },
    {
      "model": "FTXM60ZVLT",
      "cap": 6.0,
      "series": "橫綱Z系列",
      "unit_type": "壁掛式",
      "price": 30220.0
    },
    {
      "model": "FTXM71ZVLT",
      "cap": 7.2,
      "series": "橫綱Z系列",
      "unit_type": "壁掛式",
      "price": 32720.0
    },
    {
      "model": "FTXM80ZVLT",
      "cap": 8.0,
      "series": "橫綱Z系列",
      "unit_type": "壁掛式",
      "price": 37520.0
    },
    {
      "model": "FTXM90ZVLT",
      "cap": 8.7,
      "series": "橫綱Z系列",
      "unit_type": "壁掛式",
      "price": 41520.0
    },
    {
      "model": "FTHF20VAVLT",
      "cap": 2.2,
      "series": "經典VA系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTHF25VAVLT",
      "cap": 2.5,
      "series": "經典VA系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTHF30VAVLT",
      "cap": 3.0,
      "series": "經典VA系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTHF40VAVLT",
      "cap": 4.0,
      "series": "經典VA系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTHF50VAVLT",
      "cap": 5.0,
      "series": "經典VA系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTHF60VAVLT",
      "cap": 6.0,
      "series": "經典VA系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTHF71VAVLT",
      "cap": 7.2,
      "series": "經典VA系列",
      "unit_type": "壁掛式",
      "price": 0.0
    }
  ],
  "SA": [
    {
      "model": "FAA71XVLT",
      "cap": 7.2,
      "series": "商用冷專系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FCA71CVLT",
      "cap": 7.2,
      "series": "商用冷專系列",
      "unit_type": "全方吹",
      "price": 0.0
    },
    {
      "model": "FAA100XVLT",
      "cap": 9.5,
      "series": "商用冷專系列",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FCA71DVLT",
      "cap": 7.2,
      "series": "商用冷暖系列",
      "unit_type": "全方吹",
      "price": 0.0
    },
    {
      "model": "FBA71BVLT",
      "cap": 7.2,
      "series": "商用冷暖系列",
      "unit_type": "吊隱式",
      "price": 0.0
    },
    {
      "model": "FBA100BVLT",
      "cap": 10.1,
      "series": "商用冷暖系列",
      "unit_type": "吊隱式",
      "price": 0.0
    },
    {
      "model": "FCA100CVLT",
      "cap": 10.4,
      "series": "商用冷暖系列",
      "unit_type": "全方吹",
      "price": 0.0
    },
    {
      "model": "FCA125CVLT",
      "cap": 12.5,
      "series": "商用冷暖系列",
      "unit_type": "全方吹",
      "price": 0.0
    },
    {
      "model": "FBA125BVLT",
      "cap": 12.5,
      "series": "商用冷暖系列",
      "unit_type": "吊隱式",
      "price": 0.0
    },
    {
      "model": "FBA140BVLT",
      "cap": 13.3,
      "series": "商用冷暖系列",
      "unit_type": "吊隱式",
      "price": 0.0
    },
    {
      "model": "FCA140CVLT",
      "cap": 14.0,
      "series": "商用冷暖系列",
      "unit_type": "全方吹",
      "price": 0.0
    },
    {
      "model": "FCA140DVLT",
      "cap": 14.0,
      "series": "商用冷暖系列",
      "unit_type": "全方吹",
      "price": 0.0
    }
  ],
  "VRV": [
    {
      "model": "FXSQ20PAVT",
      "cap": 2.2,
      "series": "中靜壓",
      "unit_type": "吊隱式",
      "price": 34800.0
    },
    {
      "model": "FXSQ25PAVT",
      "cap": 2.8,
      "series": "中靜壓",
      "unit_type": "吊隱式",
      "price": 36100.0
    },
    {
      "model": "FXSQ32PAVT",
      "cap": 3.6,
      "series": "中靜壓",
      "unit_type": "吊隱式",
      "price": 37200.0
    },
    {
      "model": "FXSQ40PAVT",
      "cap": 4.5,
      "series": "中靜壓",
      "unit_type": "吊隱式",
      "price": 39000.0
    },
    {
      "model": "FXSQ50PAVT",
      "cap": 5.6,
      "series": "中靜壓",
      "unit_type": "吊隱式",
      "price": 40100.0
    },
    {
      "model": "FXSQ63PAVT",
      "cap": 7.1,
      "series": "中靜壓",
      "unit_type": "吊隱式",
      "price": 43700.0
    },
    {
      "model": "FXSQ80PAVT",
      "cap": 9.0,
      "series": "中靜壓",
      "unit_type": "吊隱式",
      "price": 48300.0
    },
    {
      "model": "FXSQ100PAVT",
      "cap": 11.2,
      "series": "中靜壓",
      "unit_type": "吊隱式",
      "price": 51200.0
    },
    {
      "model": "FXSQ125PAVT",
      "cap": 14.0,
      "series": "中靜壓",
      "unit_type": "吊隱式",
      "price": 53800.0
    },
    {
      "model": "FXSQ140PAVT",
      "cap": 16.0,
      "series": "中靜壓",
      "unit_type": "吊隱式",
      "price": 62700.0
    },
    {
      "model": "FXMQ20PAVT",
      "cap": 2.2,
      "series": "中高靜壓",
      "unit_type": "吊隱式",
      "price": 36000.0
    },
    {
      "model": "FXMQ25PAVT",
      "cap": 2.8,
      "series": "中高靜壓",
      "unit_type": "吊隱式",
      "price": 37100.0
    },
    {
      "model": "FXMQ32PAVT",
      "cap": 3.6,
      "series": "中高靜壓",
      "unit_type": "吊隱式",
      "price": 38300.0
    },
    {
      "model": "FXMQ40PAVT",
      "cap": 4.5,
      "series": "中高靜壓",
      "unit_type": "吊隱式",
      "price": 40100.0
    },
    {
      "model": "FXMQ50PAVT",
      "cap": 5.6,
      "series": "中高靜壓",
      "unit_type": "吊隱式",
      "price": 41100.0
    },
    {
      "model": "FXMQ63PAVT",
      "cap": 7.1,
      "series": "中高靜壓",
      "unit_type": "吊隱式",
      "price": 44700.0
    },
    {
      "model": "FXMQ80PAVT",
      "cap": 9.0,
      "series": "中高靜壓",
      "unit_type": "吊隱式",
      "price": 49300.0
    },
    {
      "model": "FXMQ100PAVT",
      "cap": 11.2,
      "series": "中高靜壓",
      "unit_type": "吊隱式",
      "price": 52300.0
    },
    {
      "model": "FXMQ125PAVT",
      "cap": 14.0,
      "series": "中高靜壓",
      "unit_type": "吊隱式",
      "price": 54800.0
    },
    {
      "model": "FXMQ140PAVT",
      "cap": 16.0,
      "series": "中高靜壓",
      "unit_type": "吊隱式",
      "price": 63800.0
    },
    {
      "model": "FXDQ20PDVT",
      "cap": 2.2,
      "series": "低靜壓(有排水泵)",
      "unit_type": "吊隱式",
      "price": 29000.0
    },
    {
      "model": "FXDQ25PDVT",
      "cap": 2.8,
      "series": "低靜壓(有排水泵)",
      "unit_type": "吊隱式",
      "price": 29900.0
    },
    {
      "model": "FXDQ32PDVT",
      "cap": 3.6,
      "series": "低靜壓(有排水泵)",
      "unit_type": "吊隱式",
      "price": 31500.0
    },
    {
      "model": "FXDQ40NDVT",
      "cap": 4.5,
      "series": "低靜壓(有排水泵)",
      "unit_type": "吊隱式",
      "price": 33400.0
    },
    {
      "model": "FXDQ50NDVT",
      "cap": 5.6,
      "series": "低靜壓(有排水泵)",
      "unit_type": "吊隱式",
      "price": 35700.0
    },
    {
      "model": "FXDQ63NDVT",
      "cap": 7.1,
      "series": "低靜壓(有排水泵)",
      "unit_type": "吊隱式",
      "price": 38100.0
    },
    {
      "model": "FXDQ20PDNVT",
      "cap": 2.2,
      "series": "低靜壓(無排水泵)",
      "unit_type": "吊隱式",
      "price": 26200.0
    },
    {
      "model": "FXDQ25PDNVT",
      "cap": 2.8,
      "series": "低靜壓(無排水泵)",
      "unit_type": "吊隱式",
      "price": 27200.0
    },
    {
      "model": "FXDQ32PDNVT",
      "cap": 3.6,
      "series": "低靜壓(無排水泵)",
      "unit_type": "吊隱式",
      "price": 28700.0
    },
    {
      "model": "FXDQ40NDNVT",
      "cap": 4.5,
      "series": "低靜壓(無排水泵)",
      "unit_type": "吊隱式",
      "price": 30600.0
    },
    {
      "model": "FXDQ50NDNVT",
      "cap": 5.6,
      "series": "低靜壓(無排水泵)",
      "unit_type": "吊隱式",
      "price": 32800.0
    },
    {
      "model": "FXDQ63NDNVT",
      "cap": 7.1,
      "series": "低靜壓(無排水泵)",
      "unit_type": "吊隱式",
      "price": 35200.0
    },
    {
      "model": "FXFQ25BVT",
      "cap": 2.8,
      "series": "全方吹(一般型)",
      "unit_type": "嵌入式",
      "price": 40000.0
    },
    {
      "model": "FXFQ32BVT",
      "cap": 3.6,
      "series": "全方吹(一般型)",
      "unit_type": "嵌入式",
      "price": 41400.0
    },
    {
      "model": "FXFQ40BVT",
      "cap": 4.5,
      "series": "全方吹(一般型)",
      "unit_type": "嵌入式",
      "price": 43100.0
    },
    {
      "model": "FXFQ50BVT",
      "cap": 5.6,
      "series": "全方吹(一般型)",
      "unit_type": "嵌入式",
      "price": 44300.0
    },
    {
      "model": "FXFQ63BVT",
      "cap": 7.1,
      "series": "全方吹(一般型)",
      "unit_type": "嵌入式",
      "price": 45400.0
    },
    {
      "model": "FXFQ80BVT",
      "cap": 9.0,
      "series": "全方吹(一般型)",
      "unit_type": "嵌入式",
      "price": 50000.0
    },
    {
      "model": "FXFQ100BVT",
      "cap": 11.2,
      "series": "全方吹(一般型)",
      "unit_type": "嵌入式",
      "price": 52900.0
    },
    {
      "model": "FXFQ125BVT",
      "cap": 14.0,
      "series": "全方吹(一般型)",
      "unit_type": "嵌入式",
      "price": 55500.0
    },
    {
      "model": "FXFQ140BVT",
      "cap": 16.0,
      "series": "全方吹(一般型)",
      "unit_type": "嵌入式",
      "price": 64400.0
    },
    {
      "model": "FXFSQ25BVT",
      "cap": 2.8,
      "series": "全方吹(智慧感應型)",
      "unit_type": "嵌入式",
      "price": 43200.0
    },
    {
      "model": "FXFSQ32BVT",
      "cap": 3.6,
      "series": "全方吹(智慧感應型)",
      "unit_type": "嵌入式",
      "price": 45300.0
    },
    {
      "model": "FXFSQ40BVT",
      "cap": 4.5,
      "series": "全方吹(智慧感應型)",
      "unit_type": "嵌入式",
      "price": 47700.0
    },
    {
      "model": "FXFSQ50BVT",
      "cap": 5.6,
      "series": "全方吹(智慧感應型)",
      "unit_type": "嵌入式",
      "price": 48400.0
    },
    {
      "model": "FXFSQ63BVT",
      "cap": 7.1,
      "series": "全方吹(智慧感應型)",
      "unit_type": "嵌入式",
      "price": 49100.0
    },
    {
      "model": "FXFSQ80BVT",
      "cap": 9.0,
      "series": "全方吹(智慧感應型)",
      "unit_type": "嵌入式",
      "price": 53100.0
    },
    {
      "model": "FXFSQ100BVT",
      "cap": 11.2,
      "series": "全方吹(智慧感應型)",
      "unit_type": "嵌入式",
      "price": 56300.0
    },
    {
      "model": "FXFSQ125BVT",
      "cap": 14.0,
      "series": "全方吹(智慧感應型)",
      "unit_type": "嵌入式",
      "price": 59500.0
    },
    {
      "model": "FXFSQ140BVT",
      "cap": 16.0,
      "series": "全方吹(智慧感應型)",
      "unit_type": "嵌入式",
      "price": 69500.0
    },
    {
      "model": "FXPQ25AVN",
      "cap": 2.8,
      "series": "單點式空調",
      "unit_type": "單點式",
      "price": 105000.0
    },
    {
      "model": "FXAQ20AVLT",
      "cap": 2.2,
      "series": "壁掛式",
      "unit_type": "壁掛式",
      "price": 34300.0
    },
    {
      "model": "FXAQ25AVLT",
      "cap": 2.8,
      "series": "壁掛式",
      "unit_type": "壁掛式",
      "price": 35400.0
    },
    {
      "model": "FXAQ32AVLT",
      "cap": 3.6,
      "series": "壁掛式",
      "unit_type": "壁掛式",
      "price": 36500.0
    },
    {
      "model": "FXAQ40AVLT",
      "cap": 4.5,
      "series": "壁掛式",
      "unit_type": "壁掛式",
      "price": 36800.0
    },
    {
      "model": "FXAQ50AVLT",
      "cap": 5.6,
      "series": "壁掛式",
      "unit_type": "壁掛式",
      "price": 37000.0
    },
    {
      "model": "FXAQ63AVLT",
      "cap": 7.1,
      "series": "壁掛式",
      "unit_type": "壁掛式",
      "price": 42100.0
    },
    {
      "model": "FXMQ48MFVJU",
      "cap": 14.0,
      "series": "外氣處理(出風溫度控制)",
      "unit_type": "吊隱式",
      "price": 108100.0
    },
    {
      "model": "FXMQ72MFVJU",
      "cap": 22.4,
      "series": "外氣處理(出風溫度控制)",
      "unit_type": "吊隱式",
      "price": 129800.0
    },
    {
      "model": "FXMQ96MFVJU",
      "cap": 28.0,
      "series": "外氣處理(出風溫度控制)",
      "unit_type": "吊隱式",
      "price": 140900.0
    },
    {
      "model": "FXMQ80AFVLT",
      "cap": 9.0,
      "series": "外氣處理(回風溫度控制)",
      "unit_type": "吊隱式",
      "price": 57100.0
    },
    {
      "model": "FXMQ140AFVLT",
      "cap": 16.0,
      "series": "外氣處理(回風溫度控制)",
      "unit_type": "吊隱式",
      "price": 79500.0
    },
    {
      "model": "FXMQ200AFVLT",
      "cap": 22.4,
      "series": "外氣處理(回風溫度控制)",
      "unit_type": "吊隱式",
      "price": 113200.0
    },
    {
      "model": "FXMQ250AFVLT",
      "cap": 28.0,
      "series": "外氣處理(回風溫度控制)",
      "unit_type": "吊隱式",
      "price": 120300.0
    },
    {
      "model": "FXHQ32MVET",
      "cap": 3.6,
      "series": "天吊式",
      "unit_type": "天吊式",
      "price": 45300.0
    },
    {
      "model": "FXHQ63MVET",
      "cap": 7.1,
      "series": "天吊式",
      "unit_type": "天吊式",
      "price": 51300.0
    },
    {
      "model": "FXHQ100MVET",
      "cap": 11.2,
      "series": "天吊式",
      "unit_type": "天吊式",
      "price": 59800.0
    },
    {
      "model": "CDXS25EAVMT",
      "cap": 2.5,
      "series": "家用系列(吊隱式)",
      "unit_type": "吊隱式",
      "price": 0.0
    },
    {
      "model": "CDXS30JVLT",
      "cap": 3.0,
      "series": "家用系列(吊隱式)",
      "unit_type": "吊隱式",
      "price": 0.0
    },
    {
      "model": "CDXS40JVLT",
      "cap": 4.0,
      "series": "家用系列(吊隱式)",
      "unit_type": "吊隱式",
      "price": 0.0
    },
    {
      "model": "CDXS50DVMT",
      "cap": 5.0,
      "series": "家用系列(吊隱式)",
      "unit_type": "吊隱式",
      "price": 0.0
    },
    {
      "model": "CDXS60DVMT",
      "cap": 6.0,
      "series": "家用系列(吊隱式)",
      "unit_type": "吊隱式",
      "price": 0.0
    },
    {
      "model": "FTXS20KVLT",
      "cap": 2.0,
      "series": "家用系列(壁掛式)",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXS25KVLT",
      "cap": 2.5,
      "series": "家用系列(壁掛式)",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "CTXS30KVLT",
      "cap": 3.0,
      "series": "家用系列(壁掛式)",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXS40JVLT",
      "cap": 4.0,
      "series": "家用系列(壁掛式)",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "CTXS50JVLT",
      "cap": 5.0,
      "series": "家用系列(壁掛式)",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "CTXS60JVLT",
      "cap": 6.0,
      "series": "家用系列(壁掛式)",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "CTXS70JVLT",
      "cap": 7.0,
      "series": "家用系列(壁掛式)",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FFQ25JVLT",
      "cap": 2.5,
      "series": "家用系列(小型多方吹)",
      "unit_type": "嵌入式",
      "price": 0.0
    },
    {
      "model": "FFQ30JVLT",
      "cap": 3.0,
      "series": "家用系列(小型多方吹)",
      "unit_type": "嵌入式",
      "price": 0.0
    },
    {
      "model": "FFQ40JVLT",
      "cap": 4.0,
      "series": "家用系列(小型多方吹)",
      "unit_type": "嵌入式",
      "price": 0.0
    },
    {
      "model": "FFQ50JVLT",
      "cap": 5.0,
      "series": "家用系列(小型多方吹)",
      "unit_type": "嵌入式",
      "price": 0.0
    },
    {
      "model": "FFQ60JVLT",
      "cap": 6.0,
      "series": "家用系列(小型多方吹)",
      "unit_type": "嵌入式",
      "price": 0.0
    },
    {
      "model": "FTXJ25NVLTW",
      "cap": 2.5,
      "series": "家用系列(歐風壁掛式)",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FTXJ41NVLTW",
      "cap": 4.1,
      "series": "家用系列(歐風壁掛式)",
      "unit_type": "壁掛式",
      "price": 0.0
    },
    {
      "model": "FXZQ20BVLT",
      "cap": 2.2,
      "series": "小型四方吹",
      "unit_type": "嵌入式",
      "price": 34500.0
    },
    {
      "model": "FXZQ25BVLT",
      "cap": 2.8,
      "series": "小型四方吹",
      "unit_type": "嵌入式",
      "price": 38500.0
    },
    {
      "model": "FXZQ32BVLT",
      "cap": 3.6,
      "series": "小型四方吹",
      "unit_type": "嵌入式",
      "price": 39900.0
    },
    {
      "model": "FXZQ40BVLT",
      "cap": 4.5,
      "series": "小型四方吹",
      "unit_type": "嵌入式",
      "price": 41100.0
    },
    {
      "model": "FXZQ50BVLT",
      "cap": 5.6,
      "series": "小型四方吹",
      "unit_type": "嵌入式",
      "price": 42300.0
    },
    {
      "model": "FXVQ250NTL",
      "cap": 28.0,
      "series": "落地箱型機",
      "unit_type": "箱型機",
      "price": 185000.0
    },
    {
      "model": "FXVQ400NTL",
      "cap": 45.0,
      "series": "落地箱型機",
      "unit_type": "箱型機",
      "price": 0.0
    },
    {
      "model": "FXVQ500NTL",
      "cap": 56.0,
      "series": "落地箱型機",
      "unit_type": "箱型機",
      "price": 0.0
    },
    {
      "model": "FXCQ20AVT",
      "cap": 2.2,
      "series": "雙向氣流",
      "unit_type": "嵌入式",
      "price": 42000.0
    },
    {
      "model": "FXCQ25AVT",
      "cap": 2.8,
      "series": "雙向氣流",
      "unit_type": "嵌入式",
      "price": 44200.0
    },
    {
      "model": "FXCQ32AVT",
      "cap": 3.6,
      "series": "雙向氣流",
      "unit_type": "嵌入式",
      "price": 45800.0
    },
    {
      "model": "FXCQ40AVT",
      "cap": 4.5,
      "series": "雙向氣流",
      "unit_type": "嵌入式",
      "price": 46900.0
    },
    {
      "model": "FXCQ50AVT",
      "cap": 5.6,
      "series": "雙向氣流",
      "unit_type": "嵌入式",
      "price": 49800.0
    },
    {
      "model": "FXCQ63AVT",
      "cap": 7.1,
      "series": "雙向氣流",
      "unit_type": "嵌入式",
      "price": 54800.0
    },
    {
      "model": "FXCQ80AVT",
      "cap": 9.0,
      "series": "雙向氣流",
      "unit_type": "嵌入式",
      "price": 61300.0
    },
    {
      "model": "FXCQ125AVT",
      "cap": 14.0,
      "series": "雙向氣流",
      "unit_type": "嵌入式",
      "price": 66800.0
    },
    {
      "model": "FXMQ200MVET",
      "cap": 22.4,
      "series": "高靜壓",
      "unit_type": "吊隱式",
      "price": 92800.0
    },
    {
      "model": "FXMQ250MVET",
      "cap": 28.0,
      "series": "高靜壓",
      "unit_type": "吊隱式",
      "price": 98000.0
    },
    {
      "model": "FXMQ200PVMT",
      "cap": 22.4,
      "series": "高靜壓(DC)",
      "unit_type": "吊隱式",
      "price": 110000.0
    },
    {
      "model": "FXMQ250PVMT",
      "cap": 28.0,
      "series": "高靜壓(DC)",
      "unit_type": "吊隱式",
      "price": 116000.0
    }
  ]
};

export const MODIFIER_VALUES = { 全內周: -0.10, 二面牆: 0.05, 西曬: 0.06, 挑高: 0.04, 頂曬: 0.05 };

// 🎯 半透明 Alpha 0.35 多邊形著色遮罩色系 (符合 Python 原型畫面風格)
export const OVERLAY_COLORS = [
  { bg: 'rgba(239, 68, 68, 0.35)', border: '#ef4444', badgeBg: '#ef4444', badgeText: '#ffffff' },
  { bg: 'rgba(59, 130, 246, 0.35)', border: '#3b82f6', badgeBg: '#3b82f6', badgeText: '#ffffff' },
  { bg: 'rgba(16, 185, 129, 0.35)', border: '#10b981', badgeBg: '#10b981', badgeText: '#ffffff' },
  { bg: 'rgba(245, 158, 11, 0.35)', border: '#f59e0b', badgeBg: '#f59e0b', badgeText: '#020617' },
  { bg: 'rgba(168, 85, 247, 0.35)', border: '#a855f7', badgeBg: '#a855f7', badgeText: '#ffffff' },
  { bg: 'rgba(236, 72, 153, 0.35)', border: '#ec4899', badgeBg: '#ec4899', badgeText: '#ffffff' }
];

// 🎯 100% 精確中心熱點標定 (16 16) 十字游標：確保滑鼠點擊基準點必為十字正中心 (含中心白點標靶)
export const CROSSHAIR_CURSOR_STYLE = `url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='32' height='32' viewBox='0 0 32 32'><line x1='16' y1='0' x2='16' y2='32' stroke='%23ef4444' stroke-width='2'/><line x1='0' y1='16' x2='32' y2='16' stroke='%23ef4444' stroke-width='2'/><circle cx='16' cy='16' r='3' fill='%23ffffff' stroke='%23ef4444' stroke-width='1.5'/></svg>") 16 16, crosshair`;

export const DYNAMIC_LOAD_RULES = [
  { keywords: ["辦公室", "辦公", "小辦公", "開放辦公", "洽談", "合約", "會議", "會客", "演講", "休息", "簡報", "作業區", "討論"], load: 630.0 },
  { keywords: ["董事長", "總經理", "主管", "經理", "執行長", "副總"], load: 550.0 },
  { keywords: ["茶水", "茶水間", "茶水區"], load: 450.0 },
  { keywords: ["男廁", "女廁", "殘障廁所", "廁所", "洗手間", "衛浴", "浴室"], load: 350.0 },
  { keywords: ["吧台", "咖啡", "咖啡區", "酒吧"], load: 700.0 },
  { keywords: ["前台", "櫃台", "大廳", "接待區"], load: 660.0 },
  { keywords: ["更衣", "更衣間", "更衣室"], load: 400.0 },
  { keywords: ["儲藏", "儲藏室", "庫房", "倉庫"], load: 450.0 },
  { keywords: ["玄關", "走道", "走廊", "通道"], load: 450.0 },
  { keywords: ["主臥", "主臥室", "套房"], load: 520.0 },
  { keywords: ["次臥", "女兒房", "小孩房", "客房", "臥室", "臥房", "店鋪"], load: 500.0 },
  { keywords: ["書房", "閱覽室"], load: 500.0 },
  { keywords: ["客廳", "起居室"], load: 550.0 },
  { keywords: ["餐廳", "用餐區", "飯廳"], load: 600.0 },
  { keywords: ["廚房", "中央廚房"], load: 700.0 },
  { keywords: ["檔案室", "檔案", "機房", "設備房", "伺服器", "電腦房"], load: 650.0 }
];

// 🎯 測試存取保護密碼 (預設為 daikin2026，可改為任意密碼或改為 "" 取消密碼)
export const SYSTEM_ACCESS_PASSWORD = "daikin2026";

// 🎯 依據最新上傳之 EQUIPMENT_Data.xlsx 建立之 系統 -> 系列別 -> 室內機型式 嚴格對應表
export const DYNAMIC_EQUIPMENT_CASCADE = {
  "RA": [
    {
      "series": "橫綱X系列",
      "types": [
        "壁掛式"
      ]
    },
    {
      "series": "橫綱Y系列",
      "types": [
        "壁掛式"
      ]
    },
    {
      "series": "橫綱Z系列",
      "types": [
        "壁掛式"
      ]
    },
    {
      "series": "大關U系列",
      "types": [
        "壁掛式"
      ]
    },
    {
      "series": "大關Z系列",
      "types": [
        "壁掛式"
      ]
    },
    {
      "series": "經典VA系列",
      "types": [
        "壁掛式"
      ]
    },
    {
      "series": "豪菁Z系列",
      "types": [
        "壁掛式"
      ]
    },
    {
      "series": "隱藏風管系列",
      "types": [
        "吊隱式"
      ]
    },
    {
      "series": "家用MULTI系列",
      "types": [
        "壁掛式",
        "吊隱式"
      ]
    },
    {
      "series": "SUPER MULTI系列",
      "types": [
        "壁掛式"
      ]
    }
  ],
  "SA": [
    {
      "series": "商用冷專系列",
      "types": [
        "壁掛式",
        "全方吹",
        "吊隱式"
      ]
    },
    {
      "series": "商用冷暖系列",
      "types": [
        "全方吹",
        "吊隱式"
      ]
    }
  ],
  "VRV": [
    {
      "series": "低靜壓(無排水泵)",
      "types": [
        "吊隱式"
      ]
    },
    {
      "series": "低靜壓(有排水泵)",
      "types": [
        "吊隱式"
      ]
    },
    {
      "series": "中靜壓",
      "types": [
        "吊隱式"
      ]
    },
    {
      "series": "中高靜壓",
      "types": [
        "吊隱式"
      ]
    },
    {
      "series": "高靜壓",
      "types": [
        "吊隱式"
      ]
    },
    {
      "series": "高靜壓(DC)",
      "types": [
        "吊隱式"
      ]
    },
    {
      "series": "外氣處理(出風溫度控制)",
      "types": [
        "吊隱式"
      ]
    },
    {
      "series": "外氣處理(回風溫度控制)",
      "types": [
        "吊隱式"
      ]
    },
    {
      "series": "單點式空調",
      "types": [
        "單點式"
      ]
    },
    {
      "series": "全方吹(一般型)",
      "types": [
        "嵌入式"
      ]
    },
    {
      "series": "全方吹(智慧感應型)",
      "types": [
        "嵌入式"
      ]
    },
    {
      "series": "壁掛式",
      "types": [
        "壁掛式"
      ]
    },
    {
      "series": "天吊式",
      "types": [
        "天吊式"
      ]
    },
    {
      "series": "落地箱型機",
      "types": [
        "箱型機"
      ]
    },
    {
      "series": "小型四方吹",
      "types": [
        "嵌入式"
      ]
    },
    {
      "series": "雙向氣流",
      "types": [
        "嵌入式"
      ]
    },
    {
      "series": "家用系列(小型多方吹)",
      "types": [
        "嵌入式"
      ]
    },
    {
      "series": "家用系列(吊隱式)",
      "types": [
        "吊隱式"
      ]
    },
    {
      "series": "家用系列(壁掛式)",
      "types": [
        "壁掛式"
      ]
    },
    {
      "series": "家用系列(歐風壁掛式)",
      "types": [
        "壁掛式"
      ]
    }
  ]
};

// 🎯 大金全系列室外機型號資料庫 (含 2MXP85ZVLT 與定價資料)
export const OUTDOOR_UNITS_DB = [
  {
    "system": "RA",
    "series": "橫綱Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM22ZVLT",
    "cap_kw": 2.2,
    "cap_index": 22.0,
    "price": 20600.0
  },
  {
    "system": "RA",
    "series": "橫綱Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM28ZVLT",
    "cap_kw": 2.8,
    "cap_index": 28.0,
    "price": 23800.0
  },
  {
    "system": "RA",
    "series": "橫綱Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM36ZVLT",
    "cap_kw": 3.5,
    "cap_index": 35.0,
    "price": 25100.0
  },
  {
    "system": "RA",
    "series": "橫綱Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM41ZVLT",
    "cap_kw": 4.1,
    "cap_index": 41.0,
    "price": 30000.0
  },
  {
    "system": "RA",
    "series": "橫綱Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM50ZVLT",
    "cap_kw": 5.0,
    "cap_index": 50.0,
    "price": 34260.0
  },
  {
    "system": "RA",
    "series": "橫綱Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM60ZVLT",
    "cap_kw": 6.0,
    "cap_index": 60.0,
    "price": 39960.0
  },
  {
    "system": "RA",
    "series": "橫綱Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM71ZVLT",
    "cap_kw": 7.2,
    "cap_index": 72.0,
    "price": 44460.0
  },
  {
    "system": "RA",
    "series": "橫綱Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM80ZVLT",
    "cap_kw": 8.0,
    "cap_index": 80.0,
    "price": 51660.0
  },
  {
    "system": "RA",
    "series": "橫綱Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM90ZVLT",
    "cap_kw": 8.7,
    "cap_index": 87.0,
    "price": 56660.0
  },
  {
    "system": "RA",
    "series": "橫綱Y系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM22YVLT",
    "cap_kw": 2.2,
    "cap_index": 22.0,
    "price": 26500.0
  },
  {
    "system": "RA",
    "series": "橫綱Y系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM28YVLT",
    "cap_kw": 2.8,
    "cap_index": 28.0,
    "price": 29700.0
  },
  {
    "system": "RA",
    "series": "橫綱Y系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM36YVLT",
    "cap_kw": 3.5,
    "cap_index": 35.0,
    "price": 32700.0
  },
  {
    "system": "RA",
    "series": "橫綱Y系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM41YVLT",
    "cap_kw": 4.1,
    "cap_index": 41.0,
    "price": 35400.0
  },
  {
    "system": "RA",
    "series": "橫綱Y系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM50YVLT",
    "cap_kw": 5.0,
    "cap_index": 50.0,
    "price": 39460.0
  },
  {
    "system": "RA",
    "series": "橫綱Y系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM60YVLT",
    "cap_kw": 6.3,
    "cap_index": 63.0,
    "price": 44960.0
  },
  {
    "system": "RA",
    "series": "橫綱Y系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM71YVLT",
    "cap_kw": 7.2,
    "cap_index": 72.0,
    "price": 47660.0
  },
  {
    "system": "RA",
    "series": "橫綱X系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM22XVLT",
    "cap_kw": 2.2,
    "cap_index": 22.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "橫綱X系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM28XVLT",
    "cap_kw": 2.8,
    "cap_index": 28.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "橫綱X系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM36XVLT",
    "cap_kw": 3.5,
    "cap_index": 35.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "橫綱X系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM41XVLT",
    "cap_kw": 4.1,
    "cap_index": 41.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "橫綱X系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM50XVLT",
    "cap_kw": 5.0,
    "cap_index": 50.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "橫綱X系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM60XVLT",
    "cap_kw": 6.0,
    "cap_index": 60.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "橫綱X系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM71XVLT",
    "cap_kw": 7.2,
    "cap_index": 72.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "橫綱X系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM80XVLT",
    "cap_kw": 8.0,
    "cap_index": 80.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "橫綱X系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXM90XVLT",
    "cap_kw": 8.7,
    "cap_index": 87.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "大關Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV22ZVLT",
    "cap_kw": 2.2,
    "cap_index": 22.0,
    "price": 19700.0
  },
  {
    "system": "RA",
    "series": "大關Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV28ZVLT",
    "cap_kw": 2.8,
    "cap_index": 28.0,
    "price": 22600.0
  },
  {
    "system": "RA",
    "series": "大關Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV36ZVLT",
    "cap_kw": 3.5,
    "cap_index": 35.0,
    "price": 24300.0
  },
  {
    "system": "RA",
    "series": "大關Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV41ZVLT",
    "cap_kw": 4.1,
    "cap_index": 41.0,
    "price": 27800.0
  },
  {
    "system": "RA",
    "series": "大關Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV50ZVLT",
    "cap_kw": 5.0,
    "cap_index": 50.0,
    "price": 33360.0
  },
  {
    "system": "RA",
    "series": "大關Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV60ZVLT",
    "cap_kw": 6.0,
    "cap_index": 60.0,
    "price": 37960.0
  },
  {
    "system": "RA",
    "series": "大關Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV71ZVLT",
    "cap_kw": 7.2,
    "cap_index": 72.0,
    "price": 41960.0
  },
  {
    "system": "RA",
    "series": "大關U系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV22UVLT",
    "cap_kw": 2.2,
    "cap_index": 22.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "大關U系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV28UVLT",
    "cap_kw": 2.8,
    "cap_index": 28.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "大關U系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV36UVLT",
    "cap_kw": 3.5,
    "cap_index": 35.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "大關U系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV41UVLT",
    "cap_kw": 4.1,
    "cap_index": 41.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "大關U系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV50UVLT",
    "cap_kw": 5.0,
    "cap_index": 50.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "大關U系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV60UVLT",
    "cap_kw": 6.0,
    "cap_index": 60.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "大關U系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXV71UVLT",
    "cap_kw": 7.2,
    "cap_index": 72.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "豪菁Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF20ZVLT",
    "cap_kw": 2.2,
    "cap_index": 22.0,
    "price": 17800.0
  },
  {
    "system": "RA",
    "series": "豪菁Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF25ZVLT",
    "cap_kw": 2.5,
    "cap_index": 25.0,
    "price": 19700.0
  },
  {
    "system": "RA",
    "series": "豪菁Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF30ZVLT",
    "cap_kw": 3.0,
    "cap_index": 30.0,
    "price": 21400.0
  },
  {
    "system": "RA",
    "series": "豪菁Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF40ZVLT",
    "cap_kw": 4.1,
    "cap_index": 41.0,
    "price": 25500.0
  },
  {
    "system": "RA",
    "series": "豪菁Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF50ZVLT",
    "cap_kw": 5.0,
    "cap_index": 50.0,
    "price": 32660.0
  },
  {
    "system": "RA",
    "series": "豪菁Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF60ZVLT",
    "cap_kw": 6.0,
    "cap_index": 60.0,
    "price": 36860.0
  },
  {
    "system": "RA",
    "series": "豪菁Z系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF71ZVLT",
    "cap_kw": 7.2,
    "cap_index": 72.0,
    "price": 40860.0
  },
  {
    "system": "RA",
    "series": "經典V系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF20VAVLT",
    "cap_kw": 2.2,
    "cap_index": 22.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "經典V系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF25VAVLT",
    "cap_kw": 2.5,
    "cap_index": 25.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "經典V系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF30VAVLT",
    "cap_kw": 3.0,
    "cap_index": 30.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "經典V系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF40VAVLT",
    "cap_kw": 4.1,
    "cap_index": 41.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "經典V系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF50VAVLT",
    "cap_kw": 5.0,
    "cap_index": 50.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "經典V系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF60VAVLT",
    "cap_kw": 6.0,
    "cap_index": 60.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "經典V系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RHF71VAVLT",
    "cap_kw": 7.2,
    "cap_index": 72.0,
    "price": 0.0
  },
  {
    "system": "RA",
    "series": "家用MULTI系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "2MXM56YVLT",
    "cap_kw": 5.6,
    "cap_index": 56.0,
    "price": 52780.0
  },
  {
    "system": "RA",
    "series": "家用MULTI系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "2MXM75YVLT",
    "cap_kw": 7.2,
    "cap_index": 72.0,
    "price": 71780.0
  },
  {
    "system": "RA",
    "series": "家用MULTI系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "3MXM90YVLT",
    "cap_kw": 8.5,
    "cap_index": 85.0,
    "price": 87180.0
  },
  {
    "system": "RA",
    "series": "家用MULTI系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "4MXM110YVLT",
    "cap_kw": 10.5,
    "cap_index": 105.0,
    "price": 99180.0
  },
  {
    "system": "RA",
    "series": "SUPER MULTI系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "2MXP50ZVLT",
    "cap_kw": 5.0,
    "cap_index": 50.0,
    "price": 41780.0
  },
  {
    "system": "RA",
    "series": "SUPER MULTI系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "2MXP85ZVLT",
    "cap_kw": 8.5,
    "cap_index": 85.0,
    "price": 74780.0
  },
  {
    "system": "VRV",
    "series": "VRVl高效率S系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RSUYQ112AVT",
    "cap_kw": 11.2,
    "cap_index": 100.0,
    "price": 115000.0
  },
  {
    "system": "VRV",
    "series": "VRV高效率S系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RSUYQ125AVT",
    "cap_kw": 12.5,
    "cap_index": 112.5,
    "price": 131000.0
  },
  {
    "system": "VRV",
    "series": "VRV高效率S系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RSUYQ140AVT",
    "cap_kw": 14.0,
    "cap_index": 125.0,
    "price": 147000.0
  },
  {
    "system": "VRV",
    "series": "VRV高效率S系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RSUYQ160AVT",
    "cap_kw": 16.0,
    "cap_index": 150.0,
    "price": 188000.0
  },
  {
    "system": "VRV",
    "series": "VRV高效率S系列",
    "outdoor_type": "側吹雙風扇",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYMQ10ARYLT",
    "cap_kw": 28.0,
    "cap_index": 250.0,
    "price": 282000.0
  },
  {
    "system": "VRV",
    "series": "VRV高效率S系列",
    "outdoor_type": "側吹雙風扇",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYMQ12ARYLT",
    "cap_kw": 33.5,
    "cap_index": 300.0,
    "price": 330000.0
  },
  {
    "system": "VRV",
    "series": "VRV IV-S系列",
    "outdoor_type": "側吹雙風扇",
    "power_supply": "3φ, 3P, 220V, 60Hz",
    "model": "RXYMQ8TTLT",
    "cap_kw": 22.4,
    "cap_index": 200.0,
    "price": 190000.0
  },
  {
    "system": "VRV",
    "series": "VRV IV-S系列",
    "outdoor_type": "側吹雙風扇",
    "power_supply": "3φ, 3P, 220V, 60Hz",
    "model": "RXYMQ10TTLT",
    "cap_kw": 25.0,
    "cap_index": 223.0,
    "price": 219000.0
  },
  {
    "system": "VRV",
    "series": "VRV III-S系列",
    "outdoor_type": "側吹雙風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXYMQ6TVET",
    "cap_kw": 15.5,
    "cap_index": 140.0,
    "price": 173000.0
  },
  {
    "system": "VRV",
    "series": "VRV III-S系列",
    "outdoor_type": "側吹雙風扇",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYMQ8TYLT",
    "cap_kw": 20.0,
    "cap_index": 180.0,
    "price": 197000.0
  },
  {
    "system": "VRV",
    "series": "VRV III-S系列",
    "outdoor_type": "側吹雙風扇",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYMQ10TYLT",
    "cap_kw": 24.0,
    "cap_index": 215.0,
    "price": 224000.0
  },
  {
    "system": "VRV",
    "series": "VRVmini+系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXYCQ4BVLT",
    "cap_kw": 11.2,
    "cap_index": 100.0,
    "price": 110500.0
  },
  {
    "system": "VRV",
    "series": "VRVmini+系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXYCQ5BVLT",
    "cap_kw": 12.5,
    "cap_index": 112.5,
    "price": 125800.0
  },
  {
    "system": "VRV",
    "series": "VRVmini+系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RXYCQ6BVLT",
    "cap_kw": 14.0,
    "cap_index": 125.0,
    "price": 141100.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ8AYLT",
    "cap_kw": 22.4,
    "cap_index": 200.0,
    "price": 235600.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ10AYLT",
    "cap_kw": 28.0,
    "cap_index": 250.0,
    "price": 28900.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ12AYLT",
    "cap_kw": 33.5,
    "cap_index": 300.0,
    "price": 33900.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ14AYLT",
    "cap_kw": 40.0,
    "cap_index": 350.0,
    "price": 390500.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ16AYLT",
    "cap_kw": 45.0,
    "cap_index": 400.0,
    "price": 443500.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ18AYLT",
    "cap_kw": 50.0,
    "cap_index": 450.0,
    "price": 497500.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ20AYLT",
    "cap_kw": 54.0,
    "cap_index": 500.0,
    "price": 547300.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ22ANYLT",
    "cap_kw": 61.5,
    "cap_index": 550.0,
    "price": 628000.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ24ANYLT",
    "cap_kw": 67.0,
    "cap_index": 600.0,
    "price": 678000.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ26ANYLT",
    "cap_kw": 73.5,
    "cap_index": 650.0,
    "price": 729500.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ28ANYLT",
    "cap_kw": 78.5,
    "cap_index": 700.0,
    "price": 782500.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ30ANYLT",
    "cap_kw": 85.0,
    "cap_index": 750.0,
    "price": 834000.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ32AN1YLT",
    "cap_kw": 90.0,
    "cap_index": 800.0,
    "price": 888000.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ34ANYLT",
    "cap_kw": 95.0,
    "cap_index": 850.0,
    "price": 941000.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ36AN1YLT",
    "cap_kw": 100.0,
    "cap_index": 900.0,
    "price": 995000.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ38ANYLT",
    "cap_kw": 104.0,
    "cap_index": 950.0,
    "price": 1044800.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ40AN1YLT",
    "cap_kw": 108.0,
    "cap_index": 1000.0,
    "price": 1094600.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ42ANYLT",
    "cap_kw": 117.0,
    "cap_index": 1050.0,
    "price": 1175500.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ44ANYLT",
    "cap_kw": 121.0,
    "cap_index": 1100.0,
    "price": 1225300.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ46ANYLT",
    "cap_kw": 130.0,
    "cap_index": 1150.0,
    "price": 1278500.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ48AN1YLT",
    "cap_kw": 135.0,
    "cap_index": 1200.0,
    "price": 1331500.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ50ANYLT",
    "cap_kw": 140.0,
    "cap_index": 1250.0,
    "price": 1385500.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ52AN1YLT",
    "cap_kw": 145.0,
    "cap_index": 1300.0,
    "price": 1438500.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ54ANYLT",
    "cap_kw": 150.0,
    "cap_index": 1350.0,
    "price": 1492500.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ56AN1YLT",
    "cap_kw": 154.0,
    "cap_index": 1400.0,
    "price": 1542300.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ58ANYLT",
    "cap_kw": 158.0,
    "cap_index": 1450.0,
    "price": 1592100.0
  },
  {
    "system": "VRV",
    "series": "VRV A系列",
    "outdoor_type": "冷專上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXQ60ANYLT",
    "cap_kw": 162.0,
    "cap_index": 1500.0,
    "price": 1641900.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ8AYLT",
    "cap_kw": 22.4,
    "cap_index": 200.0,
    "price": 282000.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ10AYLT",
    "cap_kw": 28.0,
    "cap_index": 250.0,
    "price": 346000.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ12AYLT",
    "cap_kw": 33.5,
    "cap_index": 300.0,
    "price": 406000.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ14AYLT",
    "cap_kw": 40.0,
    "cap_index": 350.0,
    "price": 469500.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ16AYLT",
    "cap_kw": 45.0,
    "cap_index": 400.0,
    "price": 529500.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ18AYLT",
    "cap_kw": 50.0,
    "cap_index": 450.0,
    "price": 595500.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ20AYLT",
    "cap_kw": 54.0,
    "cap_index": 500.0,
    "price": 655300.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ22ANYLT",
    "cap_kw": 61.5,
    "cap_index": 550.0,
    "price": 752000.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ24ANYLT",
    "cap_kw": 67.0,
    "cap_index": 600.0,
    "price": 812000.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ26ANYLT",
    "cap_kw": 73.5,
    "cap_index": 650.0,
    "price": 875500.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ28ANYLT",
    "cap_kw": 78.5,
    "cap_index": 700.0,
    "price": 935500.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ30ANYLT",
    "cap_kw": 85.0,
    "cap_index": 750.0,
    "price": 999000.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ32ANYLT",
    "cap_kw": 90.0,
    "cap_index": 800.0,
    "price": 1065000.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ34ANYLT",
    "cap_kw": 95.0,
    "cap_index": 850.0,
    "price": 1125000.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ36ANYLT",
    "cap_kw": 100.0,
    "cap_index": 900.0,
    "price": 1191000.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ38ANYLT",
    "cap_kw": 104.0,
    "cap_index": 950.0,
    "price": 1250800.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ40ANYLT",
    "cap_kw": 108.0,
    "cap_index": 1000.0,
    "price": 1310600.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ42ANYLT",
    "cap_kw": 117.0,
    "cap_index": 1050.0,
    "price": 1407500.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ44ANYLT",
    "cap_kw": 121.0,
    "cap_index": 1100.0,
    "price": 1467300.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ46ANYLT",
    "cap_kw": 130.0,
    "cap_index": 1150.0,
    "price": 1534500.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ48ANYLT",
    "cap_kw": 135.0,
    "cap_index": 1200.0,
    "price": 1594500.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ50ANYLT",
    "cap_kw": 140.0,
    "cap_index": 1250.0,
    "price": 1660500.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ52ANYLT",
    "cap_kw": 145.0,
    "cap_index": 1300.0,
    "price": 1720500.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ54ANYLT",
    "cap_kw": 150.0,
    "cap_index": 1350.0,
    "price": 1786500.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ56ANYLT",
    "cap_kw": 154.0,
    "cap_index": 1400.0,
    "price": 1846300.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ58ANYLT",
    "cap_kw": 158.0,
    "cap_index": 1450.0,
    "price": 1906100.0
  },
  {
    "system": "VRV",
    "series": "VRV H系列",
    "outdoor_type": "冷暖上吹型",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RXYQ60ANYLT",
    "cap_kw": 162.0,
    "cap_index": 1500.0,
    "price": 1965900.0
  },
  {
    "system": "SA",
    "series": "商用冷專系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RKF71XVLT",
    "cap_kw": 7.2,
    "cap_index": 72.0,
    "price": 61980.0
  },
  {
    "system": "SA",
    "series": "商用冷專系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RKF100XVLT",
    "cap_kw": 9.5,
    "cap_index": 95.0,
    "price": 87980.0
  },
  {
    "system": "SA",
    "series": "商用冷專系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RZF71DVLT",
    "cap_kw": 7.2,
    "cap_index": 72.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷專系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RZF71CVLT",
    "cap_kw": 7.2,
    "cap_index": 72.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷專系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RZF100CVLT",
    "cap_kw": 10.1,
    "cap_index": 101.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷專系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RZF125CVLT",
    "cap_kw": 12.5,
    "cap_index": 125.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷專系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RZF140CVLT",
    "cap_kw": 13.3,
    "cap_index": 133.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷專系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "3φ, 3P, 220V, 60Hz",
    "model": "RZF140DTLT",
    "cap_kw": 13.3,
    "cap_index": 133.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷專系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RZF140CYLT",
    "cap_kw": 13.3,
    "cap_index": 133.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷暖系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RZAC71EVLT",
    "cap_kw": 7.2,
    "cap_index": 72.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷暖系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RZAC100DVLT",
    "cap_kw": 10.1,
    "cap_index": 101.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷暖系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RZAC125DVLT",
    "cap_kw": 12.5,
    "cap_index": 125.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷暖系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "1φ, 220V, 60Hz",
    "model": "RZAC140DVLT",
    "cap_kw": 13.3,
    "cap_index": 133.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷暖系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "3φ, 3P, 220V, 60Hz",
    "model": "RZAC140ETLT",
    "cap_kw": 14.0,
    "cap_index": 140.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷暖系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "3φ, 4P, 380V, 60Hz",
    "model": "RZAC140DYLT",
    "cap_kw": 13.3,
    "cap_index": 133.0,
    "price": 0.0
  },
  {
    "system": "SA",
    "series": "商用冷暖系列",
    "outdoor_type": "側吹單風扇",
    "power_supply": "3φ, 3P, 220V, 60Hz",
    "model": "RZAC140DTLT",
    "cap_kw": 13.3,
    "cap_index": 133.0,
    "price": 0.0
  }
];

// 🎯 SA 商用系統 1對1 精確配對數據庫
export const SA_MATCHED_PAIRS = [
  {
    "col": "E",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FAA71XVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 7.2,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "595 × 845 × 300",
      "unit_type": "壁掛式",
      "remote_model": "-",
      "adapter_p_board": "BRP067A42",
      "wireless_receiver": "BRP072C42",
      "central_adapter_board": "kRP928BB2S",
      "price": 0.0
    },
    "outdoor": {
      "model": "RKF71XVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 7.2,
      "cap_index": 72.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "2.23",
      "mca": "10.2",
      "mfa": "15",
      "esp_pa": "-",
      "dimensions": "595×845×300",
      "outdoor_type": "側吹單風扇",
      "price": 61980.0
    }
  },
  {
    "col": "F",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FAA100XVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 9.5,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "990 × 940 × 320",
      "unit_type": "壁掛式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP084C45",
      "central_adapter_board": "kRP928BB2S",
      "price": 0.0
    },
    "outdoor": {
      "model": "RKF100XVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 9.5,
      "cap_index": 95.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "2.8",
      "mca": "15",
      "mfa": "20",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 87980.0
    }
  },
  {
    "col": "G",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FCA71DVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 7.2,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "256×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF71DVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 7.2,
      "cap_index": 72.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "2.01",
      "mca": "10",
      "mfa": "20",
      "esp_pa": "-",
      "dimensions": "608×845×300",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "H",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FCA71CVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 7.2,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "256×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF71CVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 7.2,
      "cap_index": 72.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "1.9",
      "mca": "9.5",
      "mfa": "16",
      "esp_pa": "-",
      "dimensions": "595×845×300",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "I",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FCA100CVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 10.4,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "298×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF100CVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 10.1,
      "cap_index": 101.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "3.04",
      "mca": "14",
      "mfa": "32",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "J",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FCA125CVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 12.5,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "298×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF125CVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 12.5,
      "cap_index": 125.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "4.44",
      "mca": "20.3",
      "mfa": "32",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "K",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FCA140CVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 14.0,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "298×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF140CVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 13.3,
      "cap_index": 133.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "5.41",
      "mca": "24.75",
      "mfa": "32",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "L",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FCA140DVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 14.0,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "298×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF140DTLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 13.3,
      "cap_index": 133.0,
      "power_supply": "3φ, 3P, 220V, 60Hz",
      "power_consumption_kw": "5.41",
      "mca": "14.5",
      "mfa": "25",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "M",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FCA140CVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 14.0,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "298×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF140CYLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 13.3,
      "cap_index": 133.0,
      "power_supply": "3φ, 4P, 380V, 60Hz",
      "power_consumption_kw": "5.41",
      "mca": "8.69",
      "mfa": "16",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "N",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FBA71BVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 7.2,
      "nominal_cap": "-",
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "0.23",
      "rated_current_a": "1.4",
      "esp_pa": "-",
      "dimensions": "245×1000×800",
      "unit_type": "吊隱式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF71DVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 7.2,
      "cap_index": 72.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "2.01",
      "mca": "10",
      "mfa": "20",
      "esp_pa": "-",
      "dimensions": "608×845×300",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "O",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FBA100BVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 10.1,
      "nominal_cap": "-",
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "0.3",
      "rated_current_a": "1.8",
      "esp_pa": "-",
      "dimensions": "245×1400×800",
      "unit_type": "吊隱式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF100CVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 10.1,
      "cap_index": 101.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "3.04",
      "mca": "14",
      "mfa": "32",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "P",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FBA125BVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 12.5,
      "nominal_cap": "-",
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "0.3",
      "rated_current_a": "2.3",
      "esp_pa": "-",
      "dimensions": "245×1400×800",
      "unit_type": "吊隱式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF125CVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 12.5,
      "cap_index": 125.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "4.44",
      "mca": "20.3",
      "mfa": "32",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "Q",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FBA140BVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 13.3,
      "nominal_cap": "-",
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "0.3",
      "rated_current_a": "2.3",
      "esp_pa": "-",
      "dimensions": "245×1400×800",
      "unit_type": "吊隱式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF140CVLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 13.3,
      "cap_index": 133.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "5.41",
      "mca": "24.75",
      "mfa": "32",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "R",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FBA140BVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 13.3,
      "nominal_cap": "-",
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "0.3",
      "rated_current_a": "2.3",
      "esp_pa": "-",
      "dimensions": "245×1400×800",
      "unit_type": "吊隱式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF140DTLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 13.3,
      "cap_index": 133.0,
      "power_supply": "3φ, 3P, 220V, 60Hz",
      "power_consumption_kw": "5.41",
      "mca": "14.5",
      "mfa": "25",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "S",
    "series": "商用冷專系列",
    "indoor": {
      "model": "FBA140BVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 13.3,
      "nominal_cap": "-",
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "0.3",
      "rated_current_a": "2.3",
      "esp_pa": "-",
      "dimensions": "245×1400×800",
      "unit_type": "吊隱式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZF140CYLT",
      "system": "SA",
      "series": "商用冷專系列",
      "cap_kw": 13.3,
      "cap_index": 133.0,
      "power_supply": "3φ, 4P, 380V, 60Hz",
      "power_consumption_kw": "5.41",
      "mca": "8.69",
      "mfa": "16",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "T",
    "series": "商用冷暖系列",
    "indoor": {
      "model": "FCA71DVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 7.2,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "256×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZAC71EVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 7.2,
      "cap_index": 72.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "2.1",
      "mca": "11.7",
      "mfa": "20",
      "esp_pa": "-",
      "dimensions": "608×845×300",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "U",
    "series": "商用冷暖系列",
    "indoor": {
      "model": "FCA100CVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 10.4,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "298×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZAC100DVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 10.1,
      "cap_index": 101.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "2.85",
      "mca": "14.1",
      "mfa": "25",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "V",
    "series": "商用冷暖系列",
    "indoor": {
      "model": "FCA125CVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 12.5,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "298×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZAC125DVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 12.5,
      "cap_index": 125.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "4.3",
      "mca": "19.8",
      "mfa": "32",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "W",
    "series": "商用冷暖系列",
    "indoor": {
      "model": "FCA140CVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 14.0,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "298×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZAC140DVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 13.3,
      "cap_index": 133.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "4.86",
      "mca": "22.5",
      "mfa": "32",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "X",
    "series": "商用冷暖系列",
    "indoor": {
      "model": "FCA140DVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 14.0,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "298×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZAC140ETLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 14.0,
      "cap_index": 140.0,
      "power_supply": "3φ, 3P, 220V, 60Hz",
      "power_consumption_kw": "4.7",
      "mca": "13",
      "mfa": "25",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "Y",
    "series": "商用冷暖系列",
    "indoor": {
      "model": "FCA140CVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 14.0,
      "nominal_cap": "-",
      "power_supply": "室外機供電",
      "power_consumption_kw": "-",
      "rated_current_a": "-",
      "esp_pa": "-",
      "dimensions": "298×840×840",
      "unit_type": "全方吹",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZAC140DYLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 13.3,
      "cap_index": 133.0,
      "power_supply": "3φ, 4P, 380V, 60Hz",
      "power_consumption_kw": "4.89",
      "mca": "7.76",
      "mfa": "16",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "Z",
    "series": "商用冷暖系列",
    "indoor": {
      "model": "FBA71BVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 7.2,
      "nominal_cap": "-",
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "0.23",
      "rated_current_a": "1.4",
      "esp_pa": "-",
      "dimensions": "245×1000×800",
      "unit_type": "吊隱式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZAC71EVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 7.2,
      "cap_index": 72.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "2.1",
      "mca": "11.7",
      "mfa": "20",
      "esp_pa": "-",
      "dimensions": "608×845×300",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "AA",
    "series": "商用冷暖系列",
    "indoor": {
      "model": "FBA100BVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 10.1,
      "nominal_cap": "-",
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "0.3",
      "rated_current_a": "1.8",
      "esp_pa": "-",
      "dimensions": "245×1400×800",
      "unit_type": "吊隱式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZAC100DVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 10.1,
      "cap_index": 101.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "2.85",
      "mca": "14.1",
      "mfa": "25",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "AB",
    "series": "商用冷暖系列",
    "indoor": {
      "model": "FBA125BVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 12.5,
      "nominal_cap": "-",
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "0.3",
      "rated_current_a": "2.3",
      "esp_pa": "-",
      "dimensions": "245×1400×800",
      "unit_type": "吊隱式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZAC125DVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 12.5,
      "cap_index": 125.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "4.3",
      "mca": "19.8",
      "mfa": "32",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "AC",
    "series": "商用冷暖系列",
    "indoor": {
      "model": "FBA140BVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 13.3,
      "nominal_cap": "-",
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "0.3",
      "rated_current_a": "2.3",
      "esp_pa": "-",
      "dimensions": "245×1400×800",
      "unit_type": "吊隱式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZAC140DVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 13.3,
      "cap_index": 133.0,
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "4.86",
      "mca": "22.5",
      "mfa": "32",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "AD",
    "series": "商用冷暖系列",
    "indoor": {
      "model": "FBA140BVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 13.3,
      "nominal_cap": "-",
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "0.3",
      "rated_current_a": "2.3",
      "esp_pa": "-",
      "dimensions": "245×1400×800",
      "unit_type": "吊隱式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZAC140DTLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 13.3,
      "cap_index": 133.0,
      "power_supply": "3φ, 3P, 220V, 60Hz",
      "power_consumption_kw": "4.9",
      "mca": "13.1",
      "mfa": "25",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  },
  {
    "col": "AE",
    "series": "商用冷暖系列",
    "indoor": {
      "model": "FBA140BVLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 13.3,
      "nominal_cap": "-",
      "power_supply": "1φ, 220V, 60Hz",
      "power_consumption_kw": "0.3",
      "rated_current_a": "2.3",
      "esp_pa": "-",
      "dimensions": "245×1400×800",
      "unit_type": "吊隱式",
      "remote_model": "-",
      "adapter_p_board": "-",
      "wireless_receiver": "BRP072C42-1",
      "central_adapter_board": "-",
      "price": 0.0
    },
    "outdoor": {
      "model": "RZAC140DYLT",
      "system": "SA",
      "series": "商用冷暖系列",
      "cap_kw": 13.3,
      "cap_index": 133.0,
      "power_supply": "3φ, 4P, 380V, 60Hz",
      "power_consumption_kw": "4.89",
      "mca": "7.76",
      "mfa": "16",
      "esp_pa": "-",
      "dimensions": "990×940×320",
      "outdoor_type": "側吹單風扇",
      "price": 0.0
    }
  }
];

// 🎯 控制器型號與定價資料庫
export const CONTROLLERS_DB = [
  {
    "model": "DCS301BA61",
    "name": "集中ON-OFF控制器",
    "price": 11800.0
  },
  {
    "model": "DCS302CA61",
    "name": "中央集中控制器",
    "price": 11700.0
  },
  {
    "model": "DCS302A52",
    "name": "中央集控外接監控基板",
    "price": 4700.0
  },
  {
    "model": "DCS303A61",
    "name": "家用集中控制器",
    "price": 22800.0
  },
  {
    "model": "DST301BA61",
    "name": "預定時程控制器",
    "price": 7350.0
  },
  {
    "model": "DTP401A61",
    "name": "STC集中控制器",
    "price": 25000.0
  },
  {
    "model": "DTP401A63",
    "name": "STC集中控制器電源",
    "price": 2800.0
  },
  {
    "model": "DCM601B51",
    "name": "ITM",
    "price": 108200.0
  },
  {
    "model": "DGE601A52",
    "name": "ITM擴充卡1",
    "price": 28600.0
  },
  {
    "model": "DGE601A53",
    "name": "ITM擴充卡插槽",
    "price": 19400.0
  },
  {
    "model": "DCM007A51",
    "name": "ITM WEB網路",
    "price": 19400.0
  },
  {
    "model": "DCM009A51",
    "name": "ITM BACnet協定軟體",
    "price": 27500.0
  },
  {
    "model": "DTA116A51",
    "name": "Modbus 介面",
    "price": 11500.0
  },
  {
    "model": "DMS502B51",
    "name": "BACnet 介面",
    "price": 83600.0
  },
  {
    "model": "DAM411B51",
    "name": "BACnet 擴充組",
    "price": 11700.0
  },
  {
    "model": "DCPA01",
    "name": "伶俐用轉接器",
    "price": 9200.0
  },
  {
    "model": "DCPF01",
    "name": "伶俐智控管理器",
    "price": 34700.0
  }
];
